 ...
 * 		- 70점 이상인 경우는 pass, 미만인 경우는 fail.dlqslek.
 * 		- 전체 5명 학생중 3명만 이 과목에 수강신청 했습니다.
 * 	- 학좀 결과를 아래와 같이 나올수 있도록 구현하세요.
 * 
 *		 골프과목 학점 결과 >>>
 *		--------------------------------------------------------
 *		이름				학번			필수			점수			학점	
...
public class UiMain {
	...
	GenerateGradeDisplay gradDisplay = new GenerateGradeDisplay();
	
	public static void main(String[] args) {
		
		UiMain uiMain = new UiMain();
		uiMain.createSubject();			//시험 과목 생성
		uiMain.createStundent();			//시험 학생 생성
		
		String display = uiMain.gradDisplay.getDisplay();
		System.out.println(display);
		
	}


	public void createStundent() {
		...
		// 과목별 성적 입력
		addScoreForSudent(student1, korean, 95);
		addScoreForSudent(student1, math, 56);
		addScoreForSudent(student1, golf, 95);
		
		addScoreForSudent(student2, korean, 94);
		addScoreForSudent(student2, math, 98);
		addScoreForSudent(student2, golf, 85);
		
		addScoreForSudent(student3, korean, 100);
		addScoreForSudent(student3, math, 88);
		addScoreForSudent(student3, golf, 55);
		
		
		addScoreForSudent(student4, korean, 89);
		addScoreForSudent(student4, math, 94);
		
		addScoreForSudent(student5, korean, 83);
		addScoreForSudent(student5, math, 56);
	}


	public void addScoreForSudent(Student student, Subject subject, int score) {
	...
	public void createSubject() {
		...
		//golf.setGradeType(Constant.PE_TYPE);
